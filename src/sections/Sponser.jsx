...
const Sponsors = () => {
  ...
  useGSAP(() => {
    // Advanced "Center-Out Wave" Animation
    gsap.fromTo(".sponsor-node", 
      ...
      },
      {
        ...
        stagger: {
          each: 0.05,
          from: "center", // This makes the animation ripple outward from the middle!
          grid: "auto"
        },
        scrollTrigger: {
  ...
  return (
    <section ref={containerRef} id="sponsors" className="relative min-h-screen bg-black py-24 overflow-hidden flex flex-col items-center">
      ...
      {/* --- BACKGROUND FX (Optimized for Performance) --- */}
      <div className="absolute inset-0 bg-cyber-grid opacity-20 pointer-events-none z-0" />
      
      {/* Lag-Free Radial Glow instead of heavy blur */}
      ...
      {/* --- SPONSORS GRID (Data Node Concept) --- */}
      <div className="relative z-10 container mx-auto px-4 md:px-10 max-w-7xl">
        
        {/* Responsive CSS Grid: 2 cols on mobile, 3 on tablet, 5 on desktop */}
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
          ...
          {SPONSORS_DATA.map((sponsor, index) => (
            ...
            >
              ...
              {/* The Logo (Grayscale to Color Reveal) */}
              <img 
                src={sponsor.logo} 
                alt={sponsor.name}
              ...
              {/* Bottom Tech HUD Bar (Targeting Brackets) */}
              ...
              {/* Scanning Laser Line (Activates on Hover) */}
      ...
      {/* Raw CSS for the card laser scan effect */}
      <style>{`
        @keyframes scan-down {
          0% { top: 0%; opacity: 0; }
          10% { opacity: 1; }
